...
const Counting = (() => {
    ...
    function startRound(level) {
        ...
        const obj = i18n.countingObjects[Math.floor(Math.random() * i18n.countingObjects.length)];
        ...
        if (config.addition) {
            ...
            instructionText = `${a} ${obj.emoji} and ${b} ${obj.emoji}, how many in total?`;

            // Visual objects
            objectsHTML = createObjectsDisplay(obj.emoji, a, b);
        } else {
            answer = config.min + Math.floor(Math.random() * (config.max - config.min + 1));
            instructionText = `How many ${obj.emoji} are there? Count and press the correct number!`;
