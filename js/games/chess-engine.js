/* ============================================
   ...
   chess.js: Rule engine (move generation, validation)
   Stockfish.js: AI engine (WASM, Web Worker)
   ============================================ */
...
const ChessEngine = (() => {
    ...
    // Initialize Stockfish (lazy load - only when needed)
    function initStockfish() {
        return new Promise((resolve) => {
            if (sfReady) return resolve();

            try {
                // Create Blob Worker from Stockfish.js local file
    ...
    function getStockfishMove(fen, skillLevel, depth) {
        return new Promise((resolve) => {
            if (!sfWorker || !sfReady) {
                resolve(null); // Return null if Stockfish is not available
                return;
            }
    ...
    // ── chess.js Wrapper (compatibility) ──
    ...
    // Extract 8x8 board array from chess.js game object
    ...
    // Convert Stockfish move format (e2e4) to chess.js move object
    ...
    // ── Fallback AI (if Stockfish is missing) ──
    function getFallbackMove(game, errorRate) {
        const moves = game.moves({ verbose: true });
        if (moves.length === 0) return null;

        // Random move according to error rate
        ...
        // Randomly pick from top 3 moves
