/* ============================================
   GAME: LEGO Adventure - Coding + Building
   LEGO man collects pieces, builds structures
   ============================================ */
...
const LegoAdventure = (() => {
  ...
  const BLOCK_DEFS = {
    UP:    { color: '#4ECDC4', label: 'Up', svg: dir => `<svg viewBox="0 0 32 32" width="28" height="28"><path d="M16 4l10 14H6z" fill="white"/><rect x="12" y="18" width="8" height="10" rx="2" fill="white"/></svg>` },
    DOWN:  { color: '#A55EEA', label: 'Down', svg: () => `<svg viewBox="0 0 32 32" width="28" height="28"><rect x="12" y="4" width="8" height="10" rx="2" fill="white"/><path d="M16 28l10-14H6z" fill="white"/></svg>` },
    LEFT:  { color: '#FF6B6B', label: 'Left', svg: () => `<svg viewBox="0 0 32 32" width="28" height="28"><path d="M4 16l14-10v8h10v4H18v8z" fill="white"/></svg>` },
    RIGHT: { color: '#45B7D1', label: 'Right', svg: () => `<svg viewBox="0 0 32 32" width="28" height="28"><path d="M28 16l-14-10v8H4v4h10v8z" fill="white"/></svg>` },
    PICK:  { color: '#FF9800', label: 'Pick', svg: () => `<svg viewBox="0 0 32 32" width="28" height="28"><path d="M16 4 L16 14" stroke="white" stroke-width="3" stroke-linecap="round"/><path d="M10 10 L16 16 L22 10" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><rect x="6" y="20" width="20" height="8" rx="3" fill="white"/></svg>` },
  };
  ...
  let collectedPieces; // pieces collected this round
  ...
  function renderBuildPanel() {
    ...
    panel.innerHTML = `
      <div class="lego-build-title">🏗️ Building</div>
  ...
  function renderGame() {
    ...
    inst.textContent = 'Collect LEGO pieces and reach the construction area!';
    ...
    status.innerHTML = `<span>Round: ${currentRound}/${totalRounds}</span><span>Block: ${sequence.length}/${currentLevel.maxBlocks}</span>`;
  ...
  function onAnimationDone(result) {
    if (result.success) {
      ...
      const errMsgs = {
        ...
        notReached: 'Did not reach the construction area!',
        missingPieces: 'You didn\'t collect all pieces!',
        noPiece: 'There is no piece here!',
        wrongOrder: 'Wrong order! Follow the numbered sequence!',
      };
      ...
      if (inst) {
        ...
        inst.textContent = errMsgs[result.error] || 'Error!';
      }
