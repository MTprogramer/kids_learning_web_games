/* ============================================
   GAME: Jump & Collect — Mario-style 2D Platform Adventure
   Canvas based, camera tracking, 3 themes × 4 levels = 12 stages
   - Coyote time + jump buffer + variable jump
   - AABB swept collision (including side collision)
   - Moving platform transport
   - Goomba enemies (die when stepped on)
   - 3 lives — game over when finished
   ============================================ */

const JumpCollect = (() => {
    const id = 'jump-collect';
    ...
    const LEVELS = window.JUMP_COLLECT_LEVELS || [];
    ...
    function buildDOM() {
        ...
        hud.innerHTML =
            ...
            '<div class="zt-hud-box zt-hud-level"><span class="zt-hud-label">Level</span><span>' + (currentLevelIdx + 1) + '/12</span></div>';
    ...
    function maybeShowOrientationHint(wrap) {
        ...
        overlay.innerHTML =
            '<div class="zt-rotate-card">' +
                ...
                '<div class="zt-rotate-title">For a better experience</div>' +
                '<div class="zt-rotate-text">Turn your phone sideways — platformer games are much more fun on a wide screen!</div>' +
                '<button class="zt-rotate-btn" id="zt-rotate-dismiss" type="button">Play anyway</button>' +
            '</div>';
    ...
    function showGameOver() {
        ...
        overlay.innerHTML =
            '<div class="zt-modal-card">' +
                '<div class="zt-modal-title">💥 Game Over</div>' +
                '<div class="zt-modal-sub">You lost all your lives!</div>' +
                '<div class="zt-modal-stats">Collected coins: <b>' + collectedCoins + '/' + totalCoins + '</b></div>' +
                '<div class="zt-modal-buttons">' +
                    '<button class="zt-mbtn zt-mbtn-primary" id="zt-btn-retry">🔁 Try Again</button>' +
                    '<button class="zt-mbtn zt-mbtn-secondary" id="zt-btn-back">🏠 Home</button>' +
                '</div>' +
            '</div>';
    ...
    function drawDoor() {
        ...
        ctx.textAlign = 'center';
        ctx.fillText('EXIT', sx + 18, door.y + 42);
    }
    ...
    function drawLevelBanner() {
        ...
        ctx.fillText('Level ' + (currentLevelIdx + 1) + ' — ' + theme.name, GAME_W / 2, GAME_H / 2 - 10);
        ctx.shadowBlur = 0;
        ctx.fillStyle = `rgba(255,255,255,${alpha})`;
        ctx.font = 'bold 16px "Comic Sans MS", system-ui, sans-serif';
        ctx.fillText('Collect coins, reach the door!', GAME_W / 2, GAME_H / 2 + 22);
    }
