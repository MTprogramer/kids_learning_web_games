/* ============================================
   GAME: Ice Tower — Vertical platform jumping
   Single player, endless arcade
   - Procedural platform spawn
   - Camera scrolls down
   - Wall-jump
   - Combo: consecutive jumps give points multiplier + super jump
   - Snowflake particle theme
   ============================================ */
...
const IceTower = (() => {
    ...
    const COMBO_JUMP_BOOST = 0.025;
    const COMBO_JUMP_MAX = 0.30;

    // Camera scroll
    const SCROLL_TRIGGER_RATIO = 0.45;
    const SCROLL_CATCHUP_RATE = 600;
    const SCROLL_BASE = 22;
    const SCROLL_RAMP_PER_PLATFORM = 0.18;
    ...
    function init(gameArea, level, cbs) {
        ...
        callbacks = cbs || {};
        try {
            highScore = parseInt(localStorage.getItem('bk-highscore') || '0', 10) || 0;
        } catch (e) { highScore = 0; }
        startRun();
    }
    ...
    function showComboFlash(c) {
        ...
        comboFlashEl.textContent = 'x' + c + ' COMBO!';
    ...
    function triggerGameOver() {
        ...
        if (score > highScore) {
            highScore = score;
            isNewRecord = true;
            try { localStorage.setItem('bk-highscore', String(highScore)); } catch (e) {}
        }
    ...
    function showGameOverModal(isNewRecord) {
        ...
        const retryBtn = h('button', {
            class: 'bk-mbtn bk-mbtn-primary', type: 'button', text: '↻ Try Again',
            onClick: () => {
        ...
        const exitBtn = h('button', {
            class: 'bk-mbtn bk-mbtn-secondary', type: 'button', text: '⌂ Return to Hub',
            onClick: () => { if (typeof App !== 'undefined' && App.showHub) App.showHub(); }
        });

        const scoreCell = h('b', { text: String(score) });
        if (isNewRecord) {
            scoreCell.appendChild(h('span', { class: 'bk-new-record', text: 'NEW RECORD' }));
        }
        ...
        const card = h('div', { class: 'bk-modal-card' },
            h('div', { class: 'bk-modal-title', text: '❄️ You Fell! ❄️' }),
            h('div', { class: 'bk-modal-sub', text: 'Jump higher next time!' }),
            stats,
