/* ============================================
   GAME: Penalty - Shoot at the Goal!
   SVG scene: green grass, goal, goalkeeper, ball
   ============================================ */
...
const Penalty = (() => {
    ...
    const TOTAL_SHOTS = 5;
    // 3x3 goal zones (left/center/right × top/middle/bottom)
    const ZONES = [
        { label: 'Top Left',   tx: 70,  ty: 65 },
        { label: 'Top Center',  tx: 200, ty: 55 },
        { label: 'Top Right',   tx: 330, ty: 65 },
        { label: 'Middle Left',  tx: 80,  ty: 120 },
        { label: 'Center',      tx: 200, ty: 115 },
        { label: 'Middle Right',  tx: 320, ty: 120 },
        { label: 'Bottom Left',   tx: 90,  ty: 170 },
        { label: 'Bottom Center',  tx: 200, ty: 170 },
        { label: 'Bottom Right',   tx: 310, ty: 170 },
    ];
    ...
    function render() {
        ...
        topBar.innerHTML = `
            <span class="pen-shot-info">Shot ${Math.min(shotsTaken + 1, TOTAL_SHOTS)}/${TOTAL_SHOTS}</span>
            <span class="pen-score-info">⚽ ${goals} Goals</span>
        `;
        ...
        container.innerHTML = `
            <div class="kod-gameover">
                <div class="kod-gameover-card" style="background: linear-gradient(135deg, #fd79a8, #e17055);">
                    ...
                    <h2 class="kod-gameover-title" style="color:white;">Not Enough Goals!</h2>
                    <p class="kod-gameover-sub" style="color:rgba(255,255,255,0.8);">${goals}/${TOTAL_SHOTS} goals scored. At least 3 goals required!</p>
                    <button class="kod-gameover-btn" id="pen-retry">Try Again</button>
                </div>
            </div>`;
