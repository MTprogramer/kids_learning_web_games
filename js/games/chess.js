/* ============================================
   GAME: Chess - Difficulty Selection
   chess.js + Stockfish.js
   ============================================ */

const ChessGame = (() => {
    const id = 'chess';
    // Single level (GameEngine compatibility) - difficulty selection inside the game
    const levels = [{}];

    const DIFFICULTIES = {
        'very-easy': { skill: 0,  depth: 1,  errorRate: 0.5, emoji: '😊', name: 'Very Easy', desc: 'Beginner', color: '#2ECC71' },
        'easy':      { skill: 3,  depth: 3,  errorRate: 0.3, emoji: '🙂', name: 'Easy', desc: 'Relaxed', color: '#3498DB' },
        'medium':    { skill: 8,  depth: 6,  errorRate: 0.0, emoji: '😐', name: 'Medium', desc: 'Balanced', color: '#F39C12' },
        'hard':      { skill: 15, depth: 12, errorRate: 0.0, emoji: '😤', name: 'Hard', desc: 'Challenging', color: '#E74C3C' },
        'very-hard': { skill: 20, depth: 20, errorRate: 0.0, emoji: '🔥', name: 'Very Hard', desc: 'Master', color: '#8E44AD' },
    };
    ...
    function showDifficultySelect() {
        ...
        wrap.innerHTML = `
            <div class="chess-diff-title">♟️ Chess</div>
            <p class="chess-diff-subtitle">Choose your difficulty level:</p>
    ...
    function render() {
        ...
        container.querySelector('#chess-top-info').innerHTML =
            `<span class="chess-player-name">🖥️ Computer <small style="opacity:0.6">(${diffLabel})</small></span><span class="chess-captured">${renderCapturedHTML(capturedByAI)}</span>`;
        container.querySelector('#chess-bot-info').innerHTML =
            `<span class="chess-player-name">👤 You</span><span class="chess-captured">${renderCapturedHTML(capturedByMe)}</span>`;
        ...
        if (aiThinking) statusEl.innerHTML = '<span class="chess-thinking">🤔 Thinking...</span>';
        else if (gameEnded) statusEl.textContent = '';
        else if (game.turn() === playerColor) statusEl.innerHTML = inCheck ? '<span class="chess-check">⚠️ CHECK!</span>' : '<span>♟️ Your turn</span>';
    ...
    function showLoseMessage() {
        ...
        msg.innerHTML = `<div class="chess-lose-card"><div style="font-size:3rem;">😔</div><h3>Checkmate!</h3><p>Try again!</p><button class="chess-retry-btn">Try Again</button><button class="chess-retry-btn" style="background:#3498DB;color:white;margin-left:8px;">Change Difficulty</button></div>`;
